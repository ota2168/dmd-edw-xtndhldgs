/*
		FILENAME:               AIFRPS_REL_FINANCIAL_TRANSACTION.SQL
		AUTHOR:                 CP45213 / MM23717
		SUBJECT AREA :          AGREEMENT
		SOURCE:                 AIF-RPS
		TERADATA SOURCE CODE:   72
		DESCRIPTION:            REL_FINANCIAL_TRANSACTION TABLE POPULATION FOR TRANSACTIONS
		JIRA:
...
TRUNCATE TABLE EDW_STAGING.AIFRPS_REL_FINANCIAL_TRANSACTION_PRE_WORK;

/*TRUNCATE WORK TABLE */
TRUNCATE TABLE EDW_WORK.AIFRPS_REL_FINANCIAL_TRANSACTION;
...
INSERT INTO EDW_STAGING.AIFRPS_REL_FINANCIAL_TRANSACTION_PRE_WORK
...
WITH /*+ENABLE_WITH_CLAUSE_MATERIALIZATION*/
    FINTXN_SOURCE_HASH AS
        (
			...
			FND.DIM_FUND_NATURAL_KEY_HASH_UUID,
            ...
            FROM EDW_STAGING.AIF_RPS_EDW_FINTXN_DELTA SRC
                 ...
                 (SELECT PROD_ID,
                  ...
                  WHERE ADMN_SYS_GRP_CDE = '02'
                    AND END_DT = TO_DATE('99991231', 'YYYYMMDD')
                 ) PDT1
                 ON
...
--ROW_SID,
       ...
       FALSE                                                                        AS SOURCE_DELETE_IND,
        ...
        CASE WHEN DIM_FUND_NATURAL_KEY_HASH_UUID IS NULL
            THEN UUID_GEN(NULL)::UUID
           ELSE
               DIM_FUND_NATURAL_KEY_HASH_UUID END AS DIM_FUND_NATURAL_KEY_HASH_UUID
FROM (SELECT AGREEMENT_TYPE_CDE,
             ...
             TRANSACTION_CDE,
             SYSTEM_DT,
			 NULL AS DIM_FUND_NATURAL_KEY_HASH_UUID,
...
--FTA AMOUNT TYPE
      ...
      WHERE ROWNO = 1
        AND TRANSACTION_AMT <> 0) REL_FIN
;


/* EDW_WORK.AIFASIA_REL_FINANCIAL_TRANSACTION - INSERTS
 *
 * THIS SCRIPT IS USED TO LOAD THE RECORDS THAT DON'T HAVE A RECORD IN TARGET
 *
 */

select analyze_statistics('edw_staging.aifrps_rel_financial_transaction_pre_work');


INSERT INTO EDW_WORK.AIFRPS_REL_FINANCIAL_TRANSACTION
...
FROM EDW_STAGING.AIFRPS_REL_FINANCIAL_TRANSACTION_PRE_WORK
...
select analyze_statistics('edw_work.aifrps_rel_financial_transaction');